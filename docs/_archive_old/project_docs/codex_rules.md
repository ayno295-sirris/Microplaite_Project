# Codex Rules

- Code must stay minimal.
- Do not add frameworks unless requested.
- Do not add generic abstractions.
- Do not add unnecessary classes.
- Do not add decorative comments.
- Do not hide logic.
- Do not catch exceptions silently.
- Do not leave dead code.
- During `M1` and `M2`, keep the Raspberry Pi code to config and logging only.
- Do not add hardware control in `rpi5/raspberrypi5` before the roadmap reaches it.
- Do not duplicate firmware code inside `rpi5/raspberrypi5`.
- All ESP32 firmware work belongs in `firmware/esp32_platformio`.
